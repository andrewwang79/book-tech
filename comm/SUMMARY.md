...
| 名称 | 定位 | 实时性 |
| - | - | - |
| EDP（EtherCAT Device Protocol），简称是EDP（EtherCAT | 主站和从站(伺服传感器等)的通信，实现硬件实时控制 | 纳秒 |
| ADS（Automation Device Specification） | 上位机和主站PLC通信，控制器内部所有模块的通信。倍福的中间件通信协议，是倍福系统的“软件总线” | <1毫秒 |
| EAP（EtherCAT Automation Protocol） | 主站间通信 | 1-10毫秒 |